...
export function LanguageToggle({ className, noAria = false }: { className?: string; noAria?: boolean }) {
  ...
  return (
    <Button
      ...
      onClick={switchLang}
      {...(noAria ? {} : { "aria-label": isEnglish ? "Switch to Polish" : "Przełącz na angielski" })}
    >
      {isEnglish ? (
        // EN icon (GB flag)
